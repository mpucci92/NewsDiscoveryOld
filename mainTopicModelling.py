...
def main(index,keyword,start_time,end_time):
    path_to_model = os.path.dirname(os.path.realpath(__file__)) + r'\Pretrained Models\distilbert-base-nli-stsb-mean-tokens'
...
if __name__ == '__main__':
    ...
    # Parameters #
    path_to_model = os.path.dirname(os.path.realpath(__file__)) + r'\Pretrained model\distilbert-base-nli-stsb-mean-tokens'
    index = 'news'
    keyword = ''
    start_time = 'now-24h'
    end_time = 'now'
    ...
    embeddings = transformerModel(data, path_to_model, 256)
    umap_embeddings = umapEmbeddings(15, 2, 'cosine',embeddings)
    cluster = topicClusters(15, 'euclidean', 'leaf',0.1, umap_embeddings)
